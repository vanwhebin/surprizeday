    ...
    <div class="container" v-if="!showEdit">
      <div class="header">
        ...
        <div class="header-right">
          <lin-search @query="onQueryChange" placeholder="请输入活动名"/>
        ...
        >
          ...
          <template v-for="item in filterTableColumn">
            ...
            <el-table-column
                    ...
                    v-if="item.label === '分享标题'"
            >
              <template slot-scope="props">
                <div v-if="!props.row.editFlag" class="table-edit">
                  <div @click="handleEdit(props.row)" class="content">{{ props.row.seo_title }}</div>
                  <div class="cell-icon" @click="handleCellEdit(props.row)">
                    <i class="el-icon-edit"></i>
                  </div>
                </div>
                <div v-else class="table-edit">
                  <el-input v-model="props.row.seo_title" placeholder></el-input>
            ...
            <!-- 推荐 -->
            <el-table-column label="状态" v-if="item.label === '状态'" v-bind:key="item.label">
              <template slot-scope="props">
                <el-switch
...
export default {
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { prop: 'activity_img_id', label: '封面' },
        // { prop: 'activity_img_id', label: '封面', formatter: (row, column, cellValue) => render(h){ },
        { prop: 'start_time', label: '开奖时间', formatter: (row, column, cellValue) => this.dateFormatter(row, column, cellValue) },
        { prop: 'order', label: '排序', noRepeat: true },
        { prop: 'type', label: '活动类别', formatter: (row, column, cellValue) => { console.log(FieldsState.activity_type); return FieldsState.activity_type[cellValue] } },
        { prop: 'update_time', label: '更新时间', formatter: (row, column, cellValue) => this.dateFormatter(row, column, cellValue) },
        { prop: 'status', label: '状态', noRepeat: true }
      ],
      loading: false,
  ...
  created() {
    // 获取数据
    // this.tableColumn = await this._getTableData(this.pageCount, this.currentPage)
    this._getTableData(this.pageCount, this.currentPage)
    // 操作栏
    this.operate = [
      {
        name: '编辑',
  ...
  methods: {
    // 获取数据
    async _getTableData(count, page) {
      const res = await Activity.getActivitys(count, page)
      console.log(res)
    ...
    onQueryChange(query) {
      ...
      setTimeout(() => {
        this.loading = false
        this.tableData = Activity.getDataByQuery(this.searchKeyword)
      }, 1000)
    },
    // 编辑
    editClose() {
