...
class Referral
{
    ...
    public static function handleShortLink($user, $ref)
    {
        ...
        Log::create(['log' => json_encode($ref), 'topic' => "ref"]);
        return self::quickReply($user->message_id);
    ...
    public static function quickReply($senderID)
    {
        $payload = [
            'text' => "which type of film do you prefer?",
            'quick_replies' => [
                [
                    'content_type' => 'text',
                    'title'=> 'Action',
                    'payload'=> json_encode(['ref' => 'action'])
                ],
                [
                    'content_type' => 'text',
                    'title'=> 'Comedy',
                    'payload'=> json_encode(['ref' => 'Comedy'])
                ],
                [
                    'content_type' => 'text',
                    'title'=> 'Porn',
                    'payload'=> json_encode(['ref' => 'Porn'])
                ],
            ],
        ];


        return MessageModel::sendQuickReply($senderID, $payload);
    }
