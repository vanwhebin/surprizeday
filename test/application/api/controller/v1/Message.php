            ...
            try{
                foreach ($data['entry'] as $key => $value) {
                    foreach ($value['messaging'] as $index=>$messaging) {
                        ...
                        $params = ['page' => 102, 'message' => $data, 'user' => $user->toArray()];
                        Log::create([
                            'log'  => json_encode($params),
                            'type' => 1,
                            'topic' => 'fb msg push'
                        ]);
                        $apiUrl = "http://chat.freebie-queen.com/api/v1/message/receive";
                        $apiHeader = ['Content-Type: application/json'];
                        curlHttp($apiUrl, json_encode($params),'POST', $apiHeader, true);
                        return true;
                        if (!empty($messaging['message'])) {
                            ...
                            return Referral::handleReferral($user, $messaging);
                        }
